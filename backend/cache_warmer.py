...
async def warm_cache():
    ...
    for i, question in enumerate(SEED_QUESTIONS, 1):
        ...
        try:
            ...
            # Validation happens inside query_cache.set() (single source of truth)
            if query_cache.set(question, response, context_hash=""):
                cached_count += 1
                logger.info(f"[CACHE WARMER] [{i}/{len(SEED_QUESTIONS)}] Cached: {question[:50]}...")
            else:
                failed_count += 1
                logger.warning(f"[CACHE WARMER] [{i}/{len(SEED_QUESTIONS)}] Rejected: {question[:50]}...")

        except Exception as e:
            failed_count += 1
            logger.warning(f"[CACHE WARMER] [{i}/{len(SEED_QUESTIONS)}] Failed: {e}")

        # Delay between queries to avoid rate limiting (Gemini 429s)
