...
class BaseOpenAI(BaseLLM):
    ...
    openai_api_key: Optional[SecretStr] = Field(
        alias="api_key", default_factory=secret_from_env("REDACTED", default=None)
    )
...
class OpenAI(BaseOpenAI):
    ...
    Setup:
        Install ``langchain-openai`` and set environment variable ``REDACTED``.

        .. code-block:: bash

            pip install -U langchain-openai
            export REDACTED="your-api-key"
    ...
    Key init args — client params:
        ...
        api_key: Optional[str]
            OpenAI API key. If not passed in will be read from env var REDACTED.
