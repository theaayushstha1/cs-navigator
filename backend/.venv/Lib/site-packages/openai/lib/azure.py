...
class AzureOpenAI(BaseAzureClient[httpx.Client, Stream[Any]], OpenAI):
    ...
    def __init__(
        ...
        *,
        ...
        """
        if api_key is None:
            api_key = os.environ.get("AZURE_REDACTED")
        ...
        if api_key is None and azure_ad_token is None and azure_ad_token_provider is None:
            raise OpenAIError(
                "Missing credentials. Please pass one of `api_key`, `azure_ad_token`, `azure_ad_token_provider`, or the `AZURE_REDACTED` or `AZURE_OPENAI_AD_TOKEN` environment variables."
            )
