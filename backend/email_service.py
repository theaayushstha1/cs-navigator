...
def _send_email_sync(to_email: str, subject: str, html_body: str) -> bool:
    """Send an email via SMTP (blocking). Returns True on success."""
    ...
    try:
        ...
        log.error(f"[EMAIL] Send failed: {type(e).__name__}")
        return False


async def send_email(to_email: str, subject: str, html_body: str) -> bool:
    """Non-blocking email send. Runs SMTP in a thread to avoid blocking the event loop."""
    return await asyncio.to_thread(_send_email_sync, to_email, subject, html_body)


def send_verification_email(to_email: str, token: str) -> bool:
    """Send email verification link. Points to backend API which redirects to frontend."""
    ...
    """
    return _send_email_sync(to_email, "Verify your CS Navigator account", html)


async def send_verification_email_async(to_email: str, token: str) -> bool:
    """Non-blocking version of send_verification_email."""
    verify_url = f"{API_URL}/api/verify-email?token={token}"
    html = f"""
    <div style="font-family: 'Google Sans', Arial, sans-serif; max-width: 480px; margin: 0 auto; padding: 32px;">
        <div style="text-align: center; margin-bottom: 24px;">
    ...
    """
    return await send_email(to_email, "Verify your CS Navigator account", html)
...
def send_password_reset_email(to_email: str, token: str) -> bool:
    ...
    """
    return _send_email_sync(to_email, "Reset your CS Navigator password", html)


async def send_password_reset_email_async(to_email: str, token: str) -> bool:
    """Non-blocking version of send_password_reset_email."""
    reset_url = f"{APP_URL}/reset-password?token={token}"
    html = f"""
    <div style="font-family: 'Google Sans', Arial, sans-serif; max-width: 480px; margin: 0 auto; padding: 32px;">
        <div style="text-align: center; margin-bottom: 24px;">
    ...
    """
    return await send_email(to_email, "Reset your CS Navigator password", html)
